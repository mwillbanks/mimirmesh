...
import { installFromArtifacts } from "../src/index";
...
describe("installer", () => {
	test("installs binaries from artifact directory", async () => {
			...
			`${JSON.stringify(
				{
					...
					buildId: "1.0.0+2026-03-17T00:00:00.000Z",
					artifacts: ["mimirmesh", "mimirmesh-server"],
				},
				null,
				2,
			)}\n`,
		...
		expect(await Bun.file(join(targetDir, "manifest.json")).exists()).toBe(true);
	});
});
