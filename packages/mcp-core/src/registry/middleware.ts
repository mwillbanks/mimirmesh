...
export const applyMiddleware = (middlewares: ToolMiddleware[], terminal: ToolExecutor): ToolExecutor =>
...
export const errorNormalizationMiddleware = (
  logger?: { error: (message: string, details?: string) => Promise<void> },
): ToolMiddleware => {
  return async (context, next) => {
    try {
