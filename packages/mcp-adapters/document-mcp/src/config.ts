...
const readSettings = (projectRoot: string, config: MimirmeshConfig): DocumentMcpSettings => {
	...
	const settings =
		rawSettings && typeof rawSettings === "object"
			? Object.fromEntries(Object.entries(rawSettings).filter(([, value]) => value !== undefined))
			: {};
...
const resolveDocumentPaths = (
	...
	const isLegacyRootConfig =
		settings.watchFolders.length === 1 && settings.watchFolders[0] === repoMount;
