...
describe("workflow guided cli workflows", () => {
	...
	ciSafeTest(
		"rejects mutating workflows in non-interactive terminals unless the automation-safe flag is explicit",
		async () => {
			const fixture = await createRuntimeUpgradeFixture("repairable");
			try {
				const blockedRepair = await run(
					[distBinary, "runtime", "upgrade", "repair"],
					fixture.repo,
					{
